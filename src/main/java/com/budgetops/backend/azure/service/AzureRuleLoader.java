...
public class AzureRuleLoader {
    ...
    private void loadRules() {
        try {
            ...
            Resource resource = resolver.getResource("classpath:costs/csp_azure_vm.yml");
            
            if (!resource.exists()) {
                log.warn("Azure VM rule file not found: csp_azure_vm.yml");
                return;
            }
            ...
            try (InputStream inputStream = resource.getInputStream()) {
                ...
                if (rulesList != null) {
                    ...
                    log.info("Loaded {} alert rules for Azure VM", alertRules.size());
                }
            }
        } catch (Exception e) {
            log.error("Failed to load Azure VM alert rules", e);
        }
    }
