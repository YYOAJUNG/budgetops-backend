...
public class NcpRuleLoader {
    ...
    private void loadRules() {
        try {
            ...
            Resource resource = resolver.getResource("classpath:costs/csp_ncp_server.yml");
            
            if (!resource.exists()) {
                log.warn("NCP Server rule file not found: csp_ncp_server.yml");
                return;
            }
            ...
            try (InputStream inputStream = resource.getInputStream()) {
                ...
                if (rulesList != null) {
                    ...
                    log.info("Loaded {} alert rules for NCP Server", alertRules.size());
                }
            }
        } catch (Exception e) {
            log.error("Failed to load NCP Server alert rules", e);
        }
    }
