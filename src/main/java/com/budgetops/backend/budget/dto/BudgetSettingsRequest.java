...
public record BudgetSettingsRequest(
        ...
        /**
         * 선택 사항: 계정별 예산 설정 목록.
         * 프론트엔드에서 전달하지 않으면 null/빈 리스트로 처리됩니다.
         */
        @Valid
        List<MemberAccountBudgetSetting> accountBudgets
) {

    public record MemberAccountBudgetSetting(
            @NotNull(message = "클라우드 제공자를 지정해주세요.")
            String provider,

            @NotNull(message = "클라우드 계정 ID를 지정해주세요.")
            Long accountId,
            ...
            @Min(value = 0, message = "계정별 알림 임계값은 0 이상이어야 합니다.")
            @Max(value = 100, message = "계정별 알림 임계값은 100 이하이어야 합니다.")
            Integer alertThreshold
    ) {
    }
}
