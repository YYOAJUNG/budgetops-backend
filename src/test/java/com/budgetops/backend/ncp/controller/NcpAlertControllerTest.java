...
@WebMvcTest(NcpAlertController.class)
@org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc(addFilters = false)
@DisplayName("NcpAlertController 테스트")
class NcpAlertControllerTest {

    @Autowired
    private MockMvc mockMvc;

    @MockBean
    private NcpAlertService alertService;
    ...
    void checkAllAccounts_Success() throws Exception {
        ...
        // when & then
        mockMvc.perform(post("/api/ncp/alerts/check"))
    ...
    void checkAccount_Success() throws Exception {
        ...
        // when & then
        mockMvc.perform(post("/api/ncp/alerts/check/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray());
    }
}
