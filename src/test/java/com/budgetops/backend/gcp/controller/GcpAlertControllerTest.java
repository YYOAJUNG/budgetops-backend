...
@WebMvcTest(GcpAlertController.class)
@org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc(addFilters = false)
@DisplayName("GcpAlertController 테스트")
class GcpAlertControllerTest {

    @Autowired
    private MockMvc mockMvc;

    @MockBean
    private GcpAlertService alertService;
    ...
    void checkAllAccounts_Success() throws Exception {
        // given
        given(alertService.checkAllAccounts()).willReturn(Collections.emptyList());

        // when & then
        mockMvc.perform(post("/api/gcp/alerts/check"))
    ...
    void checkAccount_Success() throws Exception {
        // given
        given(alertService.checkAccount(1L)).willReturn(Collections.emptyList());

        // when & then
        mockMvc.perform(post("/api/gcp/alerts/check/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray());
    }
}
